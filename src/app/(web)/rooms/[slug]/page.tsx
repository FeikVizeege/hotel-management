...
const RoomDetails = (props: { params: { slug: string } }) => {
    ...
    const calcNumberOfDays = () => {
        ...
        const numberOfDays = Math.ceil(timeDifference / (24 * 60 * 60 * 1000));

        return numberOfDays;
    }
    
    const handleBookNowClick = async () => {
        if (!checkinDate || !checkoutDate) return toast.error("Please provide both check-in and check-out date.");
        if (checkinDate > checkoutDate) return toast.error("Invalid check-in date.");
